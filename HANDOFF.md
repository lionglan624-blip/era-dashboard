...
| Dependency updater | daily/weekly/monthly | Scheduled auto-update of CCS (daily), CodeRabbit/PM2 (weekly), NuGet-check/Go/pip/Docker-CE/SonarQube/npm (monthly). Type A (global CLI): version check → update → version diff → email. Type B (repo): update → git diff → test → commit or revert. Type docker: `wsl -- sudo service docker start` → `apt-get upgrade docker-ce` → version diff. Type docker-image: `docker pull` → recreate container if updated → `service docker stop`. PM2 daemon reload deferred to next clean exit via detached spawn (3s delay, `exitHelpers.js`). npm-dashboard requires idle check (5 conditions). Weekly summary email (`[Dep-Summary]`) aggregates all tiers; per-tier `[Dep-Update]` email suppressed for weekly. Master switch: `UPDATE_ENABLED` env var. API: `POST /api/deps/trigger`, `GET /api/deps/status` |
